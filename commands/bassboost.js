...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0])
      return client.sendTime(
        message.channel,
        "**Veuillez fournir un niveau de bassboost. \nAvailable Levels:** `none`, `low`, `medium`, `high`"
  ...
  SlashCommand: {
    options: [
      {
        name: "level",
        description: `Veuillez fournir un niveau de bassboost. Available Levels: low, medium, high, or none`,
        value: "[level]",
        type: 3,
        required: true,
      },
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      return client.sendTime(
        interaction,
        `✅ | **Réglez le niveau d'amplification des basses sur** \`${level}\``
      );
    },
  },
};
