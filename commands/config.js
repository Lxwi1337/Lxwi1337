...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel,
        "❌ | **Vous avez mis trop de temps à répondre. Si vous souhaitez modifier les paramètres, exécutez à nouveau la commande !**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "Par quoi voulez-vous changer le préfixe ?"
      );
      ...
      await client.sendTime(
        message.channel,
        "Veuillez mentionner le rôle que vous souhaitez que les DJ aient."
      );
      ...
      client.sendTime(
        message.channel,
        "Rôle de DJ enregistré avec succès en tant que <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Vérifier le préfixe du bot",
        type: 1,
        required: false,
        options: [
          {
      ...
      },
      {
        ...
        options: [
          {
            name: "role",
            description: "Définir le rôle du DJ",
            type: 8,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Le préfixe a maintenant été défini sur \`${prefix}\``
          );
        } else {
          //has not prefix
          client.sendTime(
            interaction,
            `Le préfixe de ce serveur est \`${GuildDB.prefix}\``
          );
        }
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Modification réussie du rôle de DJ de ce serveur en ${role.name}`
          );
        } else {
          /**
          ...
          client.sendTime(
            interaction,
            `Le rôle de DJ de ce serveur est ${role.name}`
          );
        }
      }
    },
