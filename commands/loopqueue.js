...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (player.queueRepeat) {
      ...
      client.sendTime(message.channel, `:repeat: Queue Loop \`désactiver\``);
    } else {
      player.setQueueRepeat(true);
      client.sendTime(message.channel, `:repeat: Queue Loop \`activer\``);
    }
  },
  SlashCommand: {
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (player.queueRepeat) {
        player.setQueueRepeat(false);
        client.sendTime(interaction, `:repeat: **Queue Loop** \`désactiver\``);
      } else {
        player.setQueueRepeat(true);
        client.sendTime(interaction, `:repeat: **Queue Loop** \`activer\``);
      }
